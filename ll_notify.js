...
function fixCSS() {
  try {
    let textColor = getComputedStyle(document.getElementsByTagName("home-assistant")[0]).getPropertyValue("--text-primary-color").trim();
...
function _validateConfig(config) {
  try {
    ...
    switch (action) {
      case "call_service":
        ok(lodash_has(config, "domain"));
        ok(lodash_has(config, "service"));
        break;

      case "fire_event":
        ok(lodash_has(config, "event_name"));
        break;
    ...
    let msg = `Invalid action configuration for ll_notify: ${JSON.stringify(config)}`;
...
function _handleAction(hassConn, config) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "js_fire_event":
        document.dispatchEvent(new CustomEvent(config.event_name, {
...
function getHassConn() {
  return new Promise(resolve => {
    if (!window.hassConnection) {
      console.log('window.hassConnection doesnt exist. Waiting 100.');
      return delay(getHassConn());
    } else {
      console.log('window.hassConnection exists. Awaiting con', window.hassConnection);
      window.hassConnection.then(hc => {
        console.log('Finished awaiting hassConnection. retval', hc, 'conn', hc.conn, 'hc.conn.subscribEvents', hc.conn.subscribeEvents);
        resolve(hc.conn);
      });
    }
  });
...
fixCSS(); // let hassConn = document.querySelector("home-assistant").hass.connection
...
getHassConn().then(conn => {
  console.log('Got hassConn: ', conn, "subscribeEvents", conn.subscribeEvents);
  hassConn = conn;
  return hassConn;
}).then(hassConn => {
  ...
  // debugging
  set_globals(); // debug.do_5sec_test();
});
